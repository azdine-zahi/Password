"""Il s'agit d'un code Python qui implémente un système de gestion de mot de passe simple utilisant Tkinter pour l'interface graphique et 
JSON pour le stockage des données.Le système stocke les noms d'utilisateur et les mots de passe hachés dans un fichier JSON appelé "Register.json". Le code implémente les fonctionnalités suivantes :

* Vérifiez si le nom d'utilisateur et le mot de passe sont valides
* Enregistrez un nouvel utilisateur et stockez son mot de passe dans le fichier JSON
* Ajouter un nouveau mot de passe à un utilisateur existant
* Afficher la liste des mots de passe pour un utilisateur donné
* Effacez les entrées de nom d'utilisateur et de mot de passe."""
    ...
    if check_entries():
        ...
        if username in register:
            if password in register[username]["Hashes"]:
                message_label.config(text=f"Hi {username}, welcome back !", fg="red")
            else:
...
# Button frame
signing = Button(buttonFrame, text="Register", command=to_register, font=("Helvetica", 12), bg="#5E5E5E")
signing.pack(side=LEFT, padx=10)

# Button Add to add a new password to an user already registered
change = Button(buttonFrame, text="Add", command=add_password, font=("Helvetica", 12), bg="#5E5E5E")
change.pack(side=LEFT, padx=10)

# Button to clear the entries
clear = Button(buttonFrame, text="Clear", command=clear_entries, font=("Helvetica", 12), bg="red")
clear.pack(side=LEFT, padx=10)

# Button to display the user's passwords
display = Button(buttonFrame, text="DisplayPass", command=display_password, font=("Helvetica", 12), bg="#5E5E5E")
